...
const AdminBlog = () => {
    ...
    try {
      ...
      if (selectedItemEdit) {
        response = await axios.post(
          `${PORTFOLIOPOINTS.ApiBaseUrl}update-blog`,
          {
            ...values,
        ...
        response = await axios.post(
          `${PORTFOLIOPOINTS.ApiBaseUrl}add-blog`,
          values
        );
      }
  ...
  const deleteExperice = async (id) => {
    try {
      dispatch(showLoading());
      const response = await axios.post(`${PORTFOLIOPOINTS.ApiBaseUrl}delete-blog`, {
        _id: id,
      }
      );
      dispatch(hideLoading());
  ...
  return (
    <>
      ...
      {(type == "add" || selectedItemEdit) && (
        <Modal
          title={selectedItemEdit ? "Edit Post" : "Add Post"}
          open={isModalOpen}
          footer={null}
          onCancel={() => {
            setIsModalOpen(false);
